...
class SegmentationEvaluator(BaseEvaluator):
    ...
    def compute_consistency(self, inputs: torch.Tensor) -> float:
        ...
        pred_orig = torch.argmax(out_orig, dim=1)
        pred_shifted = torch.argmax(out_shifted, dim=1)
...
class ClassificationEvaluator(BaseEvaluator):
    ...
    def compute_consistency(self, inputs: torch.Tensor) -> float:
        ...
        return (torch.argmax(out_orig,1) == torch.argmax(out_shifted,1)).float().mean().item()
