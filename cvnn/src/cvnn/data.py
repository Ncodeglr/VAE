...
def get_dataloaders(cfg: dict, use_cuda: bool) -> Tuple[DataLoader, DataLoader]:
    ...
    if cfg["data"].get("recompute_statistics"):
        ...
        logger.info(f"Updated config with recomputed stats (Mean Real): {stats['mean_real']}")

    final_transform = build_transform_pipeline(cfg)

    if dataset_config["dataset_name"] == "Bretigny":
        train_dataset = _create_bretigny_dataset(cfg, "train", final_transform, dataset_config)
        valid_dataset = _create_bretigny_dataset(cfg, "valid", final_transform, dataset_config)
    elif dataset_config["dataset_name"] == "MNIST":
        base_dataset = MNIST(
            root=dataset_config["trainpath"],
            train=True,
        ...
        base_dataset = _create_dataset(cfg, final_transform, dataset_config)
    ...
    if test_indices is not None or (dataset_config["dataset_name"] in ["Bretigny", "MNIST", "CIFAR10"]):
        if dataset_config["dataset_name"] == "Bretigny":
            test_dataset = _create_bretigny_dataset(cfg, "test", final_transform, dataset_config)
        elif dataset_config["dataset_name"] == "MNIST":
            test_dataset = MNIST(
            root=dataset_config["trainpath"],
            train=False,
