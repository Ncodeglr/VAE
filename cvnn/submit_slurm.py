...
# ATTENTION : Assurez-vous que le module Python chargé est bien >= 3.12 comme requis par votre pyproject.toml
module load python/3.12.0/gcc-11.2.0 || module load python/3.9.10/gcc-11.2.0
...
# Install the library based on pyproject.toml
...
# Appel à la nouvelle CLI (cvnn)
python -m cvnn $TMPDIR/code/configs.yml -m train
...
echo "Running mode: {command}"

cd $current_dir

# On récupère le logdir généré à l'étape d'entraînement
logdir=`cat job_id/{train_job_id}`

# On utilise les arguments de la nouvelle CLI avec le flag -m et -r
python -m cvnn $logdir/config.yml -m {command} -r $logdir
...
if len(sys.argv) not in [5, 4]:
    ...
    print(f" {sys.argv[0]} <retrain|eval> logdir sourcepath")
    sys.exit(-1)
