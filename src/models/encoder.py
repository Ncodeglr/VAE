...
class ConvEncoder(nn.Module):
    def __init__(self, cfg: dict):
        ...
        # 2. Boucle de descente personnalisée
        # On boucle sur le reste de la liste channels
        ...
        # Cela permet à la couche nn.Linear suivante de savoir EXACTEMENT combien de neurones elle va recevoir en entrée, sans coder de "magic number" en dur.
        self.flattened_dim = channels[-1] * (current_size ** 2)
        ...
        # fc_logvar : Prédit l'incertitude (log-variance). 
        # Note : On utilise le log de la variance pour permettre au réseau de sortir des valeurs entre -inf et +inf (stabilité numérique) tout en garantissant une variance positive après passage à l'exponentielle (exp(logvar)).
        ...
        # En mode complexe, on prédit souvent la variance des parties Réelle et Imaginaire.
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
            # 1. Extraction spatiale
            features = self.feature_extractor(x)
            
            # 2. Aplatissement : [Batch, Canaux, H, W] -> [Batch, Canaux * H * W]
