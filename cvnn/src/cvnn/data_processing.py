...
def dual_real_to_complex_transform(tensor: Union[np.ndarray, torch.Tensor]) -> torch.Tensor:
    ...
    Args:
        tensor: Input tensor with shape (B, 2, H, W) or (2, H, W) where the second dimension contains real and imaginary parts.

    Returns:
        Complex tensor with shape (B, 1, H, W) or (1, H, W) where the single channel contains complex values.

    Raises:
        ValueError: If input tensor does not have the expected shape or channel configuration.
    """
    ...
    if tensor.ndim == 4 and tensor.shape[1] == 2:
        # Case: (B, 2, H, W)
        return torch.complex(tensor[:, 0], tensor[:, 1]).unsqueeze(1)
    elif tensor.ndim == 3 and tensor.shape[0] == 2:
        # Case: (2, H, W)
        return torch.complex(tensor[0], tensor[1]).unsqueeze(0).unsqueeze(0)
    else:
        raise ValueError(f"Expected input shape (B, 2, H, W) or (2, H, W), got {tensor.shape}")
    
def exp_amplitude_transform(
    ...
    Args:
        ...
        max_value: Maximum amplitude value in dB (e.g., 30 dB)
        min_value: Minimum amplitude value in dB (e.g., -30 dB)
...
def revert_transforms(data: Union[np.ndarray, torch.Tensor], cfg: Dict[str, Any], eps: float=1e-10) -> np.ndarray:
    ...
    if "logamplitude" in transform_names:
        data = exp_amplitude_transform(data, max_value, min_value)
